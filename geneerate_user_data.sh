...
generate_random_name() {
    # Select a random index for first and last names arrays
    first_index=$((RANDOM % ${#first_names[@]}))
    last_index=$((RANDOM % ${#last_names[@]}))
    
    # Retrieve random first and last names
...
# Function to generate a random masked phone number
generate_masked_phone() {
    # Generate a random 9-digit phone number
    phone_number=$(printf "%09d" $((100000000 + RANDOM % 900000000)))
...
# Generate user data and append to CSV file
for (( i=1; i<=200000; i++ )); do
    ...
    echo "$email,$name,$credit_score,$credit_lines,$masked_phone" >> "$csv_file"
done
